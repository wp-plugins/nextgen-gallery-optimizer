/**
 * NextGEN Gallery Optimizer Basic's Fancybox AJAX pagination.
 *
 * This is a modified version of the original ngg.js file that provides AJAX
 * pagination functionality in NextGEN Legacy/NextCellent Gallery.
 *
 * Originally, you had to use the Shutter lightbox effect in order for it to
 * work, but thanks to this modification, it can now be used with Optimizer's
 * Fancybox lightbox instead.
 *
 * Used with permission and packaged with modifications by Mark Jeldi,
	...
	if (jQuery(currentNode.parentNode).attr("id")) {
		...
		jQuery.get(ngg_ajax.callback, {p: postId, galleryid: galleryId, nggpage: pageNumber, type: "gallery"}, function (data, textStatus) {
			...
			jQuery("document").ready(function(){
				...
				// Add listeners for Optimizer's Enhanced Fancybox Lightbox.
				jQuery.nggobasicFancybox.attach();
		...
		if (jQuery(currentNode.parentNode).hasClass("ngg-imagebrowser")) {
			...
			jQuery.get(ngg_ajax.callback, {p: postId, galleryid: galleryId, pid: imageNumber, type: "browser"}, function (data, textStatus) {
				...
				jQuery("document").ready(function(){
					...
					// Add listeners for Optimizer's Enhanced Fancybox Lightbox again.
					jQuery.nggobasicFancybox.attach();
